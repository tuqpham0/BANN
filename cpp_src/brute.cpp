	...
	double				eps)			// error bound (ignored)
{
	...
	if (k > n_pts) {					// too many near neighbors?
		annError("Requesting more near neighbors than data points", ANNabort);
										...
										// run every point in data
	for (i = 0; i < n_pts; i++) {
										// compute distance from query to point
//		ANNdist div = div_component(*q, *pts[i]);
    ANNdist sqDist = annDist(dim, q, pts[i], div_component);
	...
	double				eps)			// error bound
{
	...
	for (i = 0; i < n_pts; i++) {
		...
		ANNdist sqDist = annDist(dim, pts[i], q, div_component);
