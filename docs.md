...
For ```__timed_k_search```, ```streamed = True``` alters between an older method of reading the numpy array into the C++, or the new way of accessing query points directly through pointers.
```
bann.__timed_k_search(data, query, k = 1, eps = 0, div = 'kl', streamed = True)
